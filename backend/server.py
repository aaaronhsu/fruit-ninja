        ...
        if 'type' in event and 'game_id' in event and 'timestamp' in event:
            ...
            match event['type']:
                case GameEvent.GAME_START:
                    lives = event['metadata'].get('lives', 3)
                    game_length = event['metadata'].get('game_length', 60)
                    game_id_tuple = dao.start_game(lives, game_length)
                    if game_id_tuple is None:
                        return jsonify({"message": "Failed to start game"}), 500
                    game_id = game_id_tuple[0]  # Extract the ID from the tuple
                    new_event['game_id'] = game_id
                    new_event['metadata']['game_data'] = {
                        'id': game_id,
                        'lives': lives,
...
def start_game():
    ...
    start_event = {
        "events": [{
            ...
            "game_id": None,  # game_id will be generated by dao.start_game
    ...
    # Forward the request internally to the events endpoint
    ...
    # Return the same status code and response from the events endpoint
